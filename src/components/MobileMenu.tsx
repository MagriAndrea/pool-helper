      ...
      */}
      ...
      >
        ...
        <div
          ...
          tabIndex={0}
          className={cn(
            "bg-background/90 backdrop-blur-md border border-border shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] overflow-hidden",
            // If OPEN: large rounded panel, default cursor
            // If CLOSED: small circle button, pointer cursor
            isOpen 
                ? "rounded-2xl p-4 cursor-default" 
                : "w-14 h-14 rounded-2xl flex items-center justify-center cursor-pointer hover:scale-110 active:scale-95 bg-primary text-primary-foreground shadow-lg hover:shadow-xl"
          )}
          onClick={(e) => {
            if (!isOpen) {
               setIsOpen(true);
