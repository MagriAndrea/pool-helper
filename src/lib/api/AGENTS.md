...
3b. **🧨 `shockProductIdSchema` IS NARROWER THAN `productIdSchema` ON PURPOSE.** The shock endpoint must never accept trichlor or dichlor: one shock dose of stabilized chlorine delivers tens of ppm of cyanuric acid, the exact failure this project exists to prevent. This is not duplication to tidy away — it is the API-boundary half of a guard whose other half is the `ShockProductId` type. See `src/lib/calculator/AGENTS.md` rule #9.
