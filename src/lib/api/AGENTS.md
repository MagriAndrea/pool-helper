...
7.  **`/api/v1/calculate/chlorine` (chlorine-comparison) MUST accept 0** for `price`/`weight`/`quantity`: `src/hooks/use-chlorine-comparison.ts` posts `{ price: 0, weight: 0, quantity: 0 }` on mount, and the calculator (`calculateCalciumMetrics`/`calculateSodiumMetrics`) already handles `<= 0` gracefully via `isValid: false`. Never make this schema stricter than that without re-reading the hook.
